...
class GammaInt(BoundaryCondition):
    DIMENSIONS = '[0 0 0 0 0 0 0]'

    def __init__(self, region, time, processorNo):
        super().__init__(region, time, processorNo, 'gammaInt')

        self._initialValue = self._region.initialIntermittency
    ...
    def _constructBoundaryField(self):
        ...
        for boundaryModel in BoundaryService.getBoundariesIn(self._region.rname):
            ...
            value = self._db.getValue(f'{xpath}/turbulence/transitionSST/intermittency')
            ...
            field[boundaryModel.boundary.name] = {
                ...
                BoundaryType.PRESSURE_OUTLET:       (lambda: self._constructPressureOutletGammaInt(xpath, value)),
                ...
                BoundaryType.FREE_STREAM:           (lambda: self._constructZeroGradient()),
                BoundaryType.FAR_FIELD_RIEMANN:     (lambda: self._constructZeroGradient()),
                ...
                BoundaryType.INTERFACE:             (lambda: self._constructInterfaceGammaInt(self._db.getValue(xpath + '/interface/mode'))),
    ...
    def _constructPressureOutletGammaInt(self, xpath: str, value: float):
    ...
    def _constructInterfaceGammaInt(self, spec):
